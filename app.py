        ...
        for s in data.get('streams', []):
            ...
            if ct == 'video' and not has_video:
                ...
                try:
                    ...
                    info['fps'] = (
                        round(int(n) / int(d), 2) if int(d) else 0
                    )
                except Exception:
                    pass
...
def calc_video_bitrate(
    target_mb: float, dur: float, audio_kbps: int = 128
) -> int:
    if dur <= 0:
        return 500
...
def main() -> None:
    ...
    if not audio_only:
        ...
        thumb = make_thumb_b64(str(tmp_src), ts)
        if thumb:
    ...
    reduction = (
        round((1 - target_mb / orig_mb) * 100, 1) if orig_mb else 0
    )
